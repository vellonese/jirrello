...
chrome.extension.sendMessage({cmd: "getData"}, function(response) {
    options = response
})
...
function changeWindow(event) {
    ...
    if (target.match(regex)) {
        ...
        if (partTitle) {
            ...
            var jiraPid
            var jiraUser
            chrome.extension.sendMessage({cmd: "getPidAndUserFromKey", options: options, jiraKey: jiraKey}, function(response) {
